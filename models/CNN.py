...
class CNN:
    ...
    def train(self, training_df, test_df, PARAMS=CNN_PARAMS):
        ...
        self.model.fit(train_images, train_labels,
                       batch_size=PARAMS["batch_size"], epochs=PARAMS["epochs"], verbose=PARAMS["verbose"], validation_data=None)
    ...
    def preProcess(self, train_path, test_path):
        ...
        l1_new = [row[1:] for _ in range(l1_repeat_count)
                  for row in l1_train.itertuples()]
        l2_new = [row[1:] for _ in range(l2_repeat_count)
                  for row in l2_train.itertuples()]
